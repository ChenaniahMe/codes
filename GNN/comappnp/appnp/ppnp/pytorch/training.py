...
def train_model(
        name: str, writer, column, model_class: Type[nn.Module], graph: SparseGraph, model_args: dict,
    ...
    epoch_stats = {'train': {}, 'stopping': {}, 'valtest': {}}
    trans = {'train': 'train', 'stopping': 'val', 'valtest': 'test'}
    ...
    for epoch in range(early_stopping.max_epochs):
        for phase in epoch_stats.keys():
            ...
            acc = running_corrects.item() / len(dataloaders[phase].dataset)
            epoch_stats[phase]['loss'] = loss
            epoch_stats[phase]['acc'] = acc

            res[trans[phase]+'_loss'].append(loss)
            res[trans[phase] + '_acc'].append(acc)
    ...
    for sheet in res:
        ...
        temp.to_excel(writer, sheet_name=name + '_' + sheet,
                      header=False, index=False, startcol=column)
