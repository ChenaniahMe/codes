...
def load_from_npz(file_name: str) -> SparseGraph:
    ...
    with np.load(file_name, allow_pickle=True) as loader:
        loader = dict(loader)
        if file_name.stem == 'cora':
            keys = [i for i in loader.keys()]
            for key in keys:
                if 'adj' in key or 'attr' in key:
                    new = key.replace('_', '_matrix.')
                    loader[new] = loader.pop(key)
            loader['idx_to_class'] = np.array([loader['idx_to_class'].item()[i] for i in range(7)])
            loader['class_names'] = loader.pop('idx_to_class')
            loader['idx_to_node'] = np.array([loader['idx_to_node'].item()[i]+'\n' for i in range(len(loader['labels']))])
            loader['node_names'] = loader.pop('idx_to_node')
