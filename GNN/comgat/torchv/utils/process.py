...
def load_data(dataset_str): # {'pubmed', 'citeseer', 'cora'}
    if dataset_str=="ms_academic_cs" or dataset_str=="ms_academic_phy" or dataset_str=="amazon_electronics_photo" or dataset_str=="amazon_electronics_computers":
        data = np.load(config.datapath+"/"+dataset_str+".npz")
        print("data",data)
        ...
        train_mask = np.array([0 for _ in range(labels.shape[0])])
        val_mask = np.array([0 for _ in range(labels.shape[0])])
        test_mask = np.array([0 for _ in range(labels.shape[0])])
        modify_data = True
        if modify_data:
            train_mask[:] = 0
            val_mask[:] = 0
    ...
    if to_excle:
        ...
        pd.DataFrame(train_mask).to_excel("/home/ztw/GAT/train_mask.xlsx")
        pd.DataFrame(val_mask).to_excel("/home/ztw/GAT/val_mask.xlsx")
        pd.DataFrame(test_mask).to_excel("/home/ztw/GAT/test_mask.xlsx")
    modify_data = True
    if modify_data:
        train_mask[:] = 0
        val_mask[:] = 0
