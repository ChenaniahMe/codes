...
def choseServer(vmCores, vmMemory, vmTwoNodes, serverId, reVmId):
    ...
    if vmTwoNodes:
        ...
        if serverCoreA >= needCores and serverCoreB >= needCores and serverMemoryA >= needMemory and serverMemoryB >= needMemory:
            ...
            capacity[serverId] = 0.5 * ((sysServerResource[serverId][0] + sysServerResource[serverId][1]) / (
                        serverInfos[serverTypeChosenOne][0] + serverInfos[serverTypeChosenOne][1])) \
                                 + 0.5 * ((sysServerResource[serverId][2] + sysServerResource[serverId][3]) / (
                        serverInfos[serverTypeChosenOne][2] + serverInfos[serverTypeChosenOne][3]))
...
def delVM(vmId_):
    ...
    if len(vmInfo_) == 5:
        ...
        capacity[serverId_] = 0.5 * ((sysServerResource[serverId_][0] + sysServerResource[serverId_][1]) / (
                    serverInfos[serverTypeChosenOne][0] + serverInfos[serverTypeChosenOne][1])) \
                              + 0.5 * ((sysServerResource[serverId_][2] + sysServerResource[serverId_][3]) / (
                    serverInfos[serverTypeChosenOne][2] + serverInfos[serverTypeChosenOne][3]))
...
def createVM(reqVmType, reVmId, day):
    ...
    for i in range(serverNumber):
        ...
        if choseServer(vmCores, vmMemory, vmTwoNodes, i, reVmId):
            ...
            success = 1
            break
        assert (sysServerResource[i][0] >= 0 and sysServerResource[i][1] >= 0 and sysServerResource[i][2] >= 0 and
                sysServerResource[i][3] >= 0)
    return success
...
def main():
    ...
    for i in range(int(M)):
        if is_test:
            ...
            vm = input().split(',')
        sys.stdout.flush()
        vmInfos[vm[0].strip('(').split()[0]] = [int(vm[1].split()[0]), int(vm[2].split()[0]),
                                                int(vm[3].strip(')\n').split()[0])]
