...
def get_dataset(args, split, sparse=True, **kwargs):
    ...
    # for i in range(data.train_mask.shape[0]):
    #     print("print train",i,data.train_mask[i])

    # for i in range(data.val_mask.shape[0]):
    #     print("print val",i,data.val_mask[i])

    # for i in range(data.test_mask.shape[0]):
    #     print("print test",i,data.test_mask[i])

    # print("data.train_mask", data.train_mask)
    # print("data.val_mask", data.val_mask)
    # print("data.test_mask", data.test_mask)
    # print("torch.sum(data.train_mask)", torch.sum(data.train_mask))
    print("torch.sum(data.val_mask)", torch.sum(data.val_mask))
    import numpy as np
    
    # print("torch.sum(data.test_mask)", torch.sum(data.test_mask))
    modify_data = False
    if modify_data:
        true_index = np.where(np.array(data.val_mask.cpu())==True)
        data.val_mask[true_index[0][-40:]] = False
        print("data.val_mask[140]",data.val_mask[140])
        # data.train_mask[:] = 0
        # data.val_mask[:] = 0
        # data.test_mask[:] = 0
    ...
    # print("data.test_mask", sum(data.test_mask[-1000:]))
    ...
    print("split_idx['test']",split_idx['test'])
    return dataset, data, split_idx
...
def get_planetoid_dataset(name, normalize_features=False, transform=None):
    path = osp.join(osp.dirname(osp.realpath(__file__)), '../..', 'datas', name)
